...
# ALLOW_RESOLUTION_DOWNGRADE=false  # correct above-ceiling owned copies to a within-ceiling cached release (default: false = preview / log only)
...
| `CDN_FIRST_READ_MB`          | No       | 8              | Size (MB) of the FIRST read-ahead fetch when a media file is opened — front-loads the container header + early index so a player's open/ffprobe burst needs fewer cold CDN round-trips before playback starts. Later reads use the normal 2 MB window. Clamped to 2–16 MB; set `2` to disable the enlargement. |
...
| `TRAKT_CATCHUP_LOOKBACK`     | No       | all-time       | Catch-up window: a show you've *watched* is auto-acquired to catch up on episodes aired since, if you last watched it within this window. `90d` / `12w` / `6mo` (or a bare number = days); `0`/unset = all-time. Anchored on Trakt's per-show `last_watched_at`. Only shows Trakt reports as **behind** (≥1 unwatched aired episode) are caught up — fully-watched shows are skipped. |
...
| `ALLOW_RESOLUTION_DOWNGRADE` | No       | `false`        | When `true`, the upgrade engine performs a **corrective downgrade**: when `MAX_RESOLUTION` is lowered below an owned copy's resolution, it replaces that above-ceiling copy with a ceiling-conforming **cached** release and prunes the higher-res copy — idle-gated, and only when such a replacement exists (a sole over-ceiling copy with no conforming cached release is kept). Default `false` only logs the planned downgrades (a preview) without replacing. Pure quality upgrades are unaffected by this flag. |
...
1. **Scrape**: Given an IMDB id and media type, `TorrentioScraper` queries a Stremio-compatible addon (default: Torrentio, URL auto-built from your provider token; override with `SCRAPER_ADDON_URL`) for candidate torrents. The IMDB id is taken from Trakt directly when available (Trakt includes it in every item) and only re-derived from TMDB when Trakt didn't supply one — so a title that TMDB has no IMDB id for (e.g. a TVDB-only show) is still acquirable. A title with no IMDB id anywhere can't be scraped (Torrentio is IMDB-keyed) and is skipped quietly.
...
- **Quality upgrades (movies, and shows per-episode):** re-scrapes the title (or, for a show, each owned episode individually) with Torrentio and checks whether a meaningfully-better **cached** release exists. "Meaningfully-better" follows the engine's own quality ranking (resolution dominant within the `MAX_RESOLUTION` ceiling, then source tier): the candidate must score strictly higher **and** make a concrete category jump — uncached→cached, a higher resolution, or a higher source tier at the same resolution (e.g. WEB→BluRay/REMUX). Marginal same-tier-same-resolution differences are never acted on (avoids churn), and a higher source tier is **not** chased if it would cost resolution — so the upgrade engine never swaps to a release acquisition itself would rank lower (which also rules out any flip-flop). When a better release is found it is staged (added to the debrid account) and the VFS `selection` for the title (or episode) is swapped — but only once the **library has been idle for `UPGRADE_IDLE_SECS`** (default 5 minutes without a proxy read anywhere in the library), re-checked immediately before the swap so staging latency can't catch a stream that started meanwhile. The superseded torrent is then pruned. A show's owned singleton episodes are re-scored before that season's pack handling runs, so an episode upgraded this tick can still be folded into a pack later the same tick.
- **Corrective downgrade — lowering the ceiling (opt-in, `ALLOW_RESOLUTION_DOWNGRADE`):** if you lower `MAX_RESOLUTION` below a copy you already own (a movie, a singleton episode, or a whole season pack), that copy no longer conforms — but it is never silently deleted. With `ALLOW_RESOLUTION_DOWNGRADE=false` (the default) the engine only **logs** the correction it would make (a preview). With it set to `true`, the engine replaces the above-ceiling copy with the best within-ceiling **cached** release covering the same content and prunes the old one — idle-gated exactly like a normal upgrade, and only when a conforming cached replacement actually exists (a sole over-ceiling copy with nothing to replace it is left alone rather than lost).
- **Full-season consolidation (shows, unchanged, runs after the per-episode pass):** when scattered per-episode torrents exist for a season, the engine looks for a **cached** full-season pack that covers every aired episode (per TMDB) at same-or-better source tier and resolution. If one is found, the season's episode `selection` slots are repointed to the pack and the individual episode torrents are pruned — idle-gated in the same way. Partial-season packs are never adopted, and consolidation itself never trades away quality — it is not a downgrade path.

All three operations are **idle-gated**: a slot being actively streamed is never swapped or pruned. After a service restart all slots read as idle (no in-memory handles can survive a restart).
