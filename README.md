...
5. **File probe**: For cached (immediately downloadable) content, the first 2 MB of the CDN URL is fetched and parsed for audio/subtitle tracks. If `AUDIO_LANGUAGE` or `SUBTITLE_LANGUAGE` is set and the tracks don't match, the candidate is blacklisted. Unknown or unsupported containers are accepted without probing.
