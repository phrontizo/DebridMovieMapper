...
- **Trakt-driven desired state (SP2):** A persisted `wanted` set (per user, per **(media_type, tmdb_id)** — so a movie and a show that share a numeric TMDB id are tracked independently; which sources want it — watchlist / in-progress — plus watched-state and, for shows, TMDB status, **and the IMDB id Trakt supplied**) is materialised from each enrolled user's Trakt account by `sync_trakt`. **IMDB resolution prefers Trakt's id** (`WantedRecord.imdb_id`, carried through to the `Acquire` op) over re-deriving it from TMDB's `external_ids`, because TMDB's mapping is incomplete for some titles (e.g. a TVDB-only show) — `build_acquire_request` falls back to TMDB only when Trakt gave none, and returns `Ok(None)` (skip quietly, logged at debug — Torrentio is IMDB-keyed so it can't be scraped) when no IMDB id exists anywhere, rather than WARN-ing every reconcile tick. A PURE reconcile-core (`wanted.rs`) diffs wanted-vs-owned-and-available and emits acquire/remove `Action`s, executed by `reconcile_wanted` (movies + show Trigger-B removal) and `monitor_episodes` (show episode acquisition + Trigger-A finish removal, via TMDB air dates). There is **one shared household library**: acquisition is the union across users; removal is per-user-aware.
