...
| `release.rs` | `RawCandidate` → `ReleaseInfo` parser (resolution, codec, HDR, size, seeders, cached flag, source tier); `score`/`rank` with quality prefs; hard-filters the resolution ceiling **and** cam/telesync/screener sources (quality floor), then ranks by source tier (REMUX>BluRay>WEB>HDTV) and bitrate |
