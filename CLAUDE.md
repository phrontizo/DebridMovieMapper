...
| `probe.rs` | Hand-rolled MKV/MP4 track reader; `probe_tracks` fetches the first 4 MB of a CDN URL and extracts audio/subtitle language codes; `verify` checks language requirements; corrupt/mismatch → blacklist, unknown → accept |
